...
(function () {
  ...
  const extensionMap = {
    ...
    55: "program.js",
    59: "program.cpp",
    60: "[^{}]*public\\s+(final)?\\s*class\\s+(\\w+).*|$2.java",
    61: "program.cpp",
    65: "program.cs",
    67: "program.rb",
    70: "a.py",
    73: "program.cpp",
    74: "[^{}]*public\\s+(final)?\\s*class\\s+(\\w+).*|$2.java",
    75: "program.rs",
    77: "program.kt",
    79: "program.cs",
    80: "program.c",
  ...
  async function initAppendForm(first = true, doNotRegenerateOnSubmit = false) {
    ...
    if (type === "contest" || type === "gym" || type === "group") {
      const existsProblemID = (id) =>
        $selectProblem.find("option").filter((_, el) => $(el).val() === id)
          .length;
    ...
    if (!doNotRegenerateOnSubmit) {
      delay(1000 * 60 * regenerateInterval).then(() => {
        doRegenerateOnSubmit = true;
      });
    }
    return true;
  }
